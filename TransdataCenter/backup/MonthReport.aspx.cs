    ...
    public partial class MonthReport : SmartSessionPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)//首次加载访问
            {
                ...
                else if (Flag == 2)//修理 工段暂且没做
                {
                    DataTable com = webBLL.Getrepairid(Identity);
                    if (com != null)
                    {
                        this.DDLcom.DataSource = com;
                        this.DDLcom.DataTextField = "depname";
                ...
                try
                {
                    ...
                    else if (Request.QueryString["unit"] == "GB")
                    {
                        this.DDLcom.SelectedIndex = 2;
                        DDLcomID.SelectedValue = "27";
                    }
                    else { }
        ...
        protected void BtnSearch_Click(object sender, EventArgs e)
        {
            ...
            //    Session["type"] = true;
            //}
            if (MonthreportAray.Text == "按修理公司")
            { }
            Session["upperlimit"] = 1;
            Session["lowlimit"] = (int)Session["pagesize"];
            DataTable dt = webBLL.Getdetails((bool)Session["date"], (bool)Session["repair"], DDLcomID.SelectedValue, MonthreportAray.SelectedValue, Identity, (bool)Session["type"]);
            if (dt != null)
            {
                ...
                dt = new DataTable();
                dt = webBLL.Getdetails(1, (int)Session["pagesize"], (bool)Session["date"], (bool)Session["repair"], DDLcomID.SelectedValue, MonthreportAray.SelectedValue, Identity, (bool)Session["type"]);
        ...
        protected void LastPage(object sender, EventArgs e)
        {
            ...
            DataTable dt = new DataTable();
            dt = webBLL.Getdetails((int)Session["upperlimit"], (int)Session["lowlimit"], (bool)Session["date"], (bool)Session["repair"], DDLcomID.SelectedValue, MonthreportAray.SelectedValue, Identity, (bool)Session["type"]);
            if (dt != null)
            {
                this.DGdetails.DataSource = dt;
                this.DGdetails.DataBind();
